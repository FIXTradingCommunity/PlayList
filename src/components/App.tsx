...
export default class App extends Component {
  ...
  checkTreeNode = (checked: Array<string>, targetNode: any) => {
    ...
    let removed: any[] = [];
    if (
      targetNode?.parent?.className === "lastLeaf" ||
      targetNode?.value.startsWith("codeset")
    ) {
      if (targetNode?.checked) {
        added = checked.filter((x: string) => !oldState.includes(x));
      } else {
        if (targetNode.value.startsWith("group")) {
          const numInGroup = targetNode.parent.children.find((e: any) =>
            e.value.includes("numInGroup")
          );
      ...
      if (targetNode?.checked) {
        ...
        removed = [
          ...targetNode.children.map((x: any) => x.value),
          targetNode.value,
        ];
      } else {
    ...
    if (this.playlist) {
      ...
      let updatedValues = !!added.length
        ? runner.checkValues(oldState, added)
        : runner.newUncheckValues(oldState, removed);
      if (removed.length) {
        updatedValues = runner.checkValues(
          [...updatedValues.newCheckedList],
          [...updatedValues.newCheckedList]
        );
      }
      this.setState({
          ...
          {!!this.state.treeData.length && (
            <>
              <div className="treeContainer">
                ...
                <h3 className="treeReference">
                  (All elements sorted alphabetically. Greyed out items are
                  deprecated)
                </h3>
                <div className="tree">
                  <CheckboxTree
    ...
    try {
      ...
      if (this.playlist?.duplicateValuesError?.length) {
        this.setState({
          ...
          activeCleanApp: true,
          modalAlternativeMessage:
            "Note that this version of Playlist does not support scenarios.",
        });
      ...
      for (let i = 0; i < nodes.length; i++) {
        if (nodes[i]) {
          ...
          if (doc2 && doc2.innerText === "CODESETS") {
            const allFirstCheckbox: any = nodes[i].querySelectorAll(
              ".tree > div > ol > li > ol > li > .rct-text > label > input"
            );
          ...
          if (doc2 && doc2.innerText === "FIELDS") {
            const allFirstCheckbox: any = nodes[i].querySelectorAll(
              ".tree > div > ol > li > ol > li > ol > li > .rct-text > label > input"
            );
            for (let checkbox of allFirstCheckbox) {
              checkbox.className = "disabledCheckbox";
            }
      ...
      if (this.state.treeData.length > 0 && nextState.treeData.length === 0) {
        const allFirstCheckbox: any = document.querySelectorAll(
          ".tree > div > ol > li > .rct-text > label > input"
        );
        for (let checkbox of allFirstCheckbox) {
          checkbox.disabled = true;
          checkbox.className = "disabledCheckbox";
...
*/
  ...
  private async readOrchestra(): Promise<void> {
    ...
    if (this.referenceFile && this.inputProgress && this.outputProgress) {
      ...
      this.setState({
        ReferenceFileError: "Reference Orchestra file not selected",
      });
    }
    const updatedValues = this.playlist?.checkValues(
      this.state.checkedTreeState,
  ...
  private checkIfExistInTreeRecursive = (
    tree: Array<INode>,
    value: string
  ): boolean => {
  ...
  private async readSelectorFileOrchestra(): Promise<void> {
    try {
      if (
        ...
        const filteredCheckedSelectorFileKeys = this.checkIfExistInTree(
          this.state.treeData,
          selectorKeys.newCheckedSelectorFileKeys
        );
        // Removing duplicated elements from the checked list.
        const dataArr = new Set([
          ...filteredCheckedSelectorFileKeys,
          ...this.state.checkedTreeState,
        ]);
        const updatedValues = [...dataArr];
        this.setState({
          readingFile: false,
          checkedTreeState: updatedValues || [],
          showCircularProgress: false,
          isSelectorFile: false,
        });
        if (selectorKeys.newNumInGroupSelectorFileKeys.length) {
          const updatedNumInGroupValues = this.playlist?.checkValues(
            this.state.checkedTreeState,
            selectorKeys.newNumInGroupSelectorFileKeys
          );
          this.setState({
            readingFile: false,
            checkedTreeState: updatedNumInGroupValues?.newCheckedList || [],
          });
        }
      }
    } catch (err) {
  ...
  private async createOrchestra(): Promise<void> {
    ...
    if (this.playlist && this.orchestraFileName) {
      ...
      this.setState({
        ...
        referenceFileError:
          !this.referenceFile && !this.playlist && "FIX log file not selected",
      });
    }
